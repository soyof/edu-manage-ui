<template>
  <div class="book-manage-container common-action-column">
    <!-- 使用通用表格页面组件 -->
    <TablePage
      ref="tablePageRef"
    ...
    >
      ...
      <template #search-form="{ form }">
        <el-col :span="6">
          <el-form-item label="著作标题">
            <el-input v-model="form.title" placeholder="请输入著作标题" clearable />
          </el-form-item>
        </el-col>
        <el-col :span="6">
        ...
        <!-- 著作发布日期 -->
        <el-col :span="6">
          <el-form-item label="著作发布日期">
            <el-date-picker
              v-model="form.bookPublishDateRange"
              type="daterange"
              rangeSeparator="~"
      ...
      >
        <template #default="scope">
          <el-tag :type="BookStatus.getTagType(scope.row.publishStatus)">
            {{ BookStatus.getName(scope.row.publishStatus) }}
          </el-tag>
        </template>
      </el-table-column>
      <el-table-column
        prop="bookPublishDate"
        label="著作发布日期"
        width="120"
        showOverflowTooltip
      />
...
const initialSearchForm = {
  ...
  author: '',
  publishStatus: '',
  bookPublishDateRange: [] as string[],
...
const isDeleteButtonDisabled = computed(() => {
  return selectedBooks.value.length === 0 || selectedBooks.value.some(item => item.publishStatus === '1')
})

// 获取著作数据的方法（适配tablePage组件的接口）
const fetchBookData = (params: any) => {
  return service.post('/api/book/list', params)
}

// 处理表格选择变化
const handleSelectionChange = (selection: BookItem[]) => {
  selectedBooks.value = selection
}

// 查看著作
const handleView = (row: BookItem) => {
  const tabTitle = `著作详情【${row.title}】`
  router.push({
    path: '/bookDetail',
    query: {
      mode: 'view',
...
const handleDelete = (row?: BookItem) => {
  ...
  if (isBatchDelete && selectedBooks.value.length === 0) {
    ElMessage.warning('请选择要删除的著作')
    return
  }

  // 检查是否包含已发布的著作
  if (isBatchDelete && selectedBooks.value.some(item => item.publishStatus === '1')) {
  ...
  const ids = isBatchDelete
    ? selectedBooks.value.map(item => item.id)
    : row!.id

  // 确认提示信息
  const confirmMessage = isBatchDelete
    ? `确认删除选中的 ${selectedBooks.value.length} 条著作信息吗？`
    : '确认删除该著作信息吗？'
  ...
  ElMessageBox.confirm(confirmMessage, '提示', {
    ...
    service.post('/api/book/delete', { ids })
      .then(() => {
...
// 处理著作发布状态（发布/下线）
const handlePublishStatus = (row: BookItem, action: 'publish' | 'unpublish') => {
  const isPublish = action === 'publish'
  const actionText = isPublish ? '发布' : '下线'

  ElMessageBox.confirm(`确认${actionText}该著作吗？`, '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: isPublish ? 'info' : 'warning'
  }).then(() => {
    service.post('/api/book/publish', {
      id: row.id,
      action
    }).then(() => {
